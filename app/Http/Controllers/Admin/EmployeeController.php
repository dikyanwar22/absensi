...
class EmployeeController extends Controller
{
    ...
    public function idCard(Employee $employee)
    {
        ...
        // Foto karyawan -> base64 untuk DomPDF
        $photoBase64 = null;
        if ($employee->photo) {
            $paths = [
                public_path('uploads/' . $employee->photo),
                public_path($employee->photo),
        ...
        try {
            ...
            try {
                ...
                $qrSvg = \SimpleSoftwareIO\QrCode\Facades\QrCode::format('svg')->size(200)->margin(1)->errorCorrection('H')->generate($qrData);
                // SVG sebagai base64 agar DomPDF bisa render
        ...
        return $pdf->stream("ID-Card-{$employee->user->nik}-{$employee->user->name}.pdf");
    }
